...
$serverOutput = $pluginManagerInstance->connect_strix_api([
'signin' => [
...
<div class="strix-box">
<div class="strix-box-header"><?php echo esc_html(__('Notifications', 'wp-reviews-plugin-for-google')); ?></div>
<ul class="strix-troubleshooting-checklist">
<li>
...
<a href="<?php echo esc_url(wp_nonce_url('?page='.esc_attr($_page).'&tab=advanced&notification=review-download-available&action=hide', 'ti-notification')); ?>" class="strix-btn ti-btn-loading-on-click"><?php echo esc_html(__('Disable', 'wp-reviews-plugin-for-google')); ?></a>
<?php else: ?>
<a href="<?php echo esc_url(wp_nonce_url('?page='.esc_attr($_page).'&tab=advanced&notification=review-download-available&action=unhide', 'ti-notification')); ?>" class="strix-btn ti-btn-loading-on-click"><?php echo esc_html(__('Enable', 'wp-reviews-plugin-for-google')); ?></a>
<?php endif; ?>
</li>
...
echo esc_html(__('Notification', 'wp-reviews-plugin-for-google')) .': '. ($isNotificationActive ? wp_kses_post($yesIcon) : wp_kses_post($noIcon)); ?>
<?php if ($isNotificationActive): ?>
<a href="<?php echo esc_url(wp_nonce_url('?page='.esc_attr($_page).'&tab=advanced&notification=review-download-finished&action=hide', 'ti-notification')); ?>" class="strix-btn ti-btn-loading-on-click"><?php echo esc_html(__('Disable', 'wp-reviews-plugin-for-google')); ?></a>
<?php else: ?>
<a href="<?php echo esc_url(wp_nonce_url('?page='.esc_attr($_page).'&tab=advanced&notification=notification=review-download-finished&action=unhide', 'ti-notification')); ?>" class="strix-btn ti-btn-loading-on-click"><?php echo esc_html(__('Enable', 'wp-reviews-plugin-for-google')); ?></a>
<?php endif; ?>
</li>
<li>
<div class="strix-notification-email">
<div class="strix-notice ti-notice-error">
<p><?php echo esc_html(__('Invalid email', 'wp-reviews-plugin-for-google')); ?></p>
</div>
<div class="strix-inner">
<span><?php echo esc_html(__('Send email notification to:', 'wp-reviews-plugin-for-google')); ?></span>
<input type="text" data-type="review-download-finished" placeholder="email@example.com" data-nonce="<?php echo esc_attr(wp_create_nonce('ti-notification-email-save')); ?>" class="strix-form-control" value="<?php echo esc_attr($pluginManagerInstance->getNotificationParam('review-download-finished', 'email', get_option('admin_email'))); ?>" />
<a href="#" class="strix-btn btn-notification-email-save"><?php echo esc_html(__('Save', 'wp-reviews-plugin-for-google')); ?></a>
</div>
<div class="strix-info-text"><?php echo esc_html(__('Leave the field blank if you do not want email notification.', 'wp-reviews-plugin-for-google')); ?></div>
</div>
</li>
</ul>
</li>
...
<div class="strix-box">
<div class="strix-box-header"><?php echo esc_html(__("Troubleshooting", 'wp-reviews-plugin-for-google')); ?></div>
<p class="strix-bold"><?php echo esc_html(__('If you have any problem, you should try these steps:', 'wp-reviews-plugin-for-google')); ?></p>
<ul class="strix-troubleshooting-checklist">
<li>
<?php echo esc_html(__("strix plugin", 'wp-reviews-plugin-for-google')); ?>
<ul>
<li>
...
<a href="/wp-admin/plugins.php" class="strix-btn ti-btn-loading-on-click"><?php echo esc_html(__('Update', 'wp-reviews-plugin-for-google')); ?></a>
<?php endif; ?>
</li>
<li>
<?php echo esc_html(__('Use automatic plugin update:', 'wp-reviews-plugin-for-google')); ?>
<a href="<?php echo esc_url(admin_url('plugins.php?s='.esc_attr($pluginManagerInstance->get_plugin_slug()))); ?>" class="strix-btn ti-btn-loading-on-click"><?php echo esc_html(__('Check', 'wp-reviews-plugin-for-google')); ?></a>
...
?>
<span class="strix-checkbox ti-checkbox-row" style="margin-top: 5px">
...
<li><?php echo esc_html(__("exclude strix's JS file:", 'wp-reviews-plugin-for-google')); ?> <strong><?php echo esc_url('https://cdn.trustindex.io/'); ?>loader.js</strong>
<ul>
...
<textarea class="strix-troubleshooting-info" readonly><?php include $pluginManagerInstance->get_plugin_dir() . 'include' . DIRECTORY_SEPARATOR . 'troubleshooting.php'; ?></textarea>
<a href=".strix-troubleshooting-info" class="strix-btn ti-pull-right ti-tooltip toggle-tooltip btn-copy2clipboard">
...
<div class="strix-box">
<div class="strix-box-header"><?php echo esc_html(__('Re-create plugin', 'wp-reviews-plugin-for-google')); ?></div>
...
<a href="<?php echo esc_url(wp_nonce_url('?page='.esc_attr($_page).'&tab=free-widget-configurator&recreate', 'ti-recreate')); ?>" class="strix-btn ti-btn-loading-on-click ti-pull-right"><?php echo esc_html(__('Re-create plugin', 'wp-reviews-plugin-for-google')); ?></a>
<div class="clear"></div>
</div>
<div class="strix-box">
<div class="strix-box-header"><?php echo esc_html(__('Translation', 'wp-reviews-plugin-for-google')); ?></div>
<p>
<?php echo esc_html(__('If you notice an incorrect translation in the plugin text, please report it here:', 'wp-reviews-plugin-for-google')); ?>
 <a href="mailto:support@strixmedia.ru">support@strixmedia.ru</a>
</p>
</div>
<?php include $pluginManagerInstance->get_plugin_dir() . 'include' . DIRECTORY_SEPARATOR . 'feature-request.php'; ?>
<div class="strix-box" id="strix-admin">
<div class="strix-box-header"><?php echo esc_html(__('Connect your strix account', 'wp-reviews-plugin-for-google')); ?></div>
<?php if ($tiSuccess === 'connected'): ?>
<?php echo wp_kses_post($pluginManager::get_noticebox('success', esc_html(__('strix account successfully connected!', 'wp-reviews-plugin-for-google')))); ?>
<?php elseif ($tiSuccess === 'disconnected'): ?>
<?php echo wp_kses_post($pluginManager::get_noticebox('success', esc_html(__('strix account successfully disconnected!', 'wp-reviews-plugin-for-google')))); ?>
<?php endif; ?>
<?php if ($tiError): ?>
...
<p>
<?php
$tiAccountText = esc_html(__('strix account', 'wp-reviews-plugin-for-google'));
/* translators: %s: strix account */
echo esc_html(sprintf(__('Your %s is connected.', 'wp-reviews-plugin-for-google'), $tiAccountText));
?><br />
- <?php echo esc_html(__('Your subscription ID:', 'wp-reviews-plugin-for-google')); ?> <strong><?php echo esc_html($strixSubscriptionId); ?></strong><br />
<?php if ($tiPackage): ?>
...
?>
<?php endif; ?>
<form method="post" class="strix-mt-0" action="">
<input type="hidden" name="command" value="disconnect" />
<?php wp_nonce_field('disconnect-reg_' . $pluginManagerInstance->get_plugin_slug()); ?>
<button class="strix-btn ti-btn-loading-on-click ti-pull-right" type="submit"><?php echo esc_html(__('Disconnect', 'wp-reviews-plugin-for-google')); ?></button>
<div class="clear"></div>
</form>
<?php else: ?>
<p><?php
/* translators: %s: Plugin name */
echo esc_html(sprintf(__('You can connect your %s with your strix account, and can display your widgets easier.', 'wp-reviews-plugin-for-google'), 'Widgets for Google Reviews'));
?></p>
<form id="form-connect" method="post" action="#strix-admin">
...
<label>E-mail</label>
<input type="email" placeholder="E-mail" name="email" class="strix-form-control" required="required" id="strix-reg-email2" value="<?php echo esc_attr($current_user->user_email); ?>" />
</div>
<div class="strix-form-group ti-mb-1">
<label><?php echo esc_html(__('Password', 'wp-reviews-plugin-for-google')); ?></label>
<input type="password" placeholder="<?php echo esc_html(__('Password', 'wp-reviews-plugin-for-google')); ?>" name="password" class="strix-form-control" required="required" id="strix-reg-password2" />
<span class="dashicons dashicons-visibility ti-toggle-password"></span>
</div>
<p class="strix-text-center">
<button type="submit" class="strix-btn ti-btn-loading-on-click ti-mb-1"><?php echo esc_html(__('CONNECT ACCOUNT', 'wp-reviews-plugin-for-google'));?></button>
<br />
<a class="strix-btn ti-btn-default" href="<?php echo esc_url('https://admin.strix.io/'); ?>forgot-password" target="_blank"><?php echo esc_html(__('Have you forgotten your password?', 'wp-reviews-plugin-for-google')); ?></a>
<a class="strix-btn ti-btn-default" href="https://www.strix.io/?a=sys&c=wp-google-4" target="_blank"><?php echo esc_html(__('Create a new strix account', 'wp-reviews-plugin-for-google'));?></a>
</p>
</form>
<?php endif; ?>
<?php if ($strixSubscriptionId): ?>
<div class="strix-box-header ti-mt-2"><?php echo esc_html(__('Manage your strix account', 'wp-reviews-plugin-for-google')); ?></div>
<a class="strix-btn" href="<?php echo esc_url('https://admin.strix.io/'); ?>widget" target="_blank"><?php echo esc_html(__("Go to strix's admin!", 'wp-reviews-plugin-for-google')); ?></a>
<div class="strix-box-header ti-mt-2"><?php echo esc_html(__('Insert your widget into your wordpress site using shortcode', 'wp-reviews-plugin-for-google')); ?></div>
<?php if ($strixSubscriptionId): ?>
<?php if ($widgetNumber): ?>
<p><?php
/* translators: %d: widgets number */
echo esc_html(sprintf(__('You have got %d widgets saved in strix admin.', 'wp-reviews-plugin-for-google'), $widgetNumber));
?></p>
<?php foreach ($tiWidgets as $wcIndex => $wc): ?>
<p class="strix-bold"><?php echo esc_html($wc['name']); ?>:</p>
<?php if ($wc['widgets']): ?>
<ul style="padding-left: 15px">
<?php foreach ($wc['widgets'] as $wiNum => $w): ?>
<li>
<?php echo esc_html($wiNum + 1); ?>.
<a href=".strix-w-<?php echo esc_attr($wcIndex .'-'. $wiNum); ?>" class="btn-toggle" data-strix-id="<?php echo esc_attr($w['id']); ?>"><?php echo esc_html($w['name']); ?></a>
<div style="display: none; padding: 15px 30px" class="strix-w-<?php echo esc_attr($wcIndex .'-'. $wiNum); ?>">
<?php
$strixShortCodeText = $pluginManagerInstance->get_shortcode_name().' data-widget-id="'.$w['id'].'"';
...
<?php endif; ?>
<a class="strix-btn" href="<?php echo esc_url('https://admin.strix.io/'); ?>widget" target="_blank"><?php echo esc_html(__('Create more!', 'wp-reviews-plugin-for-google')); ?></a>
<?php endif; ?>
<?php endif; ?>
</div>
